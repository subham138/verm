...
ReportRouter.get('/incident_report', async (req, res) => {
    var frm_dt = req.query.frm_dt,
        ...
        whr = `a.inc_type_id=b.id AND a.inc_location_id=c.id AND a.initial_tier_id=d.id AND a.created_by=e.email AND DATE(a.inc_dt) >= "${frm_dt}" AND DATE(a.inc_dt) <= "${to_dt}"`,
...
ReportRouter.get('/board_report', async (req, res) => {
    ...
    switch (board_id) {
		...
		case "4": // PROB BOARD
            // table_name = `(
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '12-03 A.M' AS time, '1' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '00:00:00' AND a.time <= '03:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	UNION
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '04-07 A.M' AS time, '2' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '04:00:00' AND a.time <= '07:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	UNION
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '08-11 A.M' AS time, '3' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '08:00:00' AND a.time <= '11:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	UNION
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '12-03 P.M' AS time, '4' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '12:00:00' AND a.time <= '15:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	UNION
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '04-07 P.M' AS time, '5' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '16:00:00' AND a.time <= '19:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	UNION
			// 	SELECT a.prob_cat_id, b.name AS prob_cat, '08-11 P.M' AS time, '6' as serial, SUM(a.value) AS value FROM td_prob_board a, md_prob_category b WHERE a.prob_cat_id=b.id AND a.time >= '20:00:00' AND a.time <= '23:59:59' AND a.inc_id = "${inc_id}" GROUP BY a.prob_cat_id
			// 	)a`;
      // select = `prob_cat_id, prob_cat, time, serial, value`;
      // whr = null;
      // order = 'ORDER BY serial, prob_cat_id';
      // dt = await F_Select(select, table_name, whr, order);
...
ReportRouter.get('/activation_report', async (req, res) => {
    var frm_dt = req.query.frm_dt,
        ...
        whr = `a.created_by=b.email AND a.inc_id=c.id AND a.team_id=d.id AND d.team_type_id=e.id AND DATE(a.created_at) >= "${frm_dt}" AND DATE(a.created_at) <= "${to_dt}"`,
